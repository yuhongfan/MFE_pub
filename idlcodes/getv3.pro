PRO getv3,datadir,it,v3,x1_v3,x2_v3,x3_v3,time_v3

;Purpose: Read one v3 data array inside domain (no ghost zones) at time step it
;
;Usage: getv3,datadir,it,v3,x1_v3,x2_v3,x3_v3,time_v3
;datadir (input): full path of the mfe output data directory
;it (input): time step number
;v3 (return): array for v3
;x1_v3 (return): dimension-1 grid position of the v3 array
;x2_v3 (return): dimension-2 grid position of the v3 array
;x3_v3 (return): dimension-3 grid position of the v3 array
;time_v3 (return): time
;
...
filev3=datadir+'/v3_'+strtrim(string(it,format='(i4.4)'),1)+'.dat'
readdata,filev3,data,time,in,jn,kn
v3=data(is:ie,js:je,ks:kep1)
time_v3=time
