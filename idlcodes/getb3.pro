PRO getb3,datadir,it,b3,x1_b3,x2_b3,x3_b3,time_b3

;Purpose: Read one b3 data array inside domain (no ghost zones) at time step it
;
;Usage: getb3,datadir,it,b3,x1_b3,x2_b3,x3_b3,time_b3
;datadir (input): full path of the mfe output data directory
;it (input): time step number
;b3 (return): array for b3
;x1_b3 (return): dimension-1 grid position of the b3
;x2_b3 (return): dimension-2 grid position of the b3
;x3_b3 (return): dimension-3 grid position of the b3
;time_b3 (return): time
;
...
fileb3=datadir+'/b3_'+strtrim(string(it,format='(i4.4)'),1)+'.dat'
readdata,fileb3,data,time,in,jn,kn
b3=data(is:ie,js:je,ks:kep1)
time_b3=time
