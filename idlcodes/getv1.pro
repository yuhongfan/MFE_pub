PRO getv1,datadir,it,v1,x1_v1,x2_v1,x3_v1,time_v1

;Purpose: Read one v1 data array inside domain (no ghost zones) at time step it
;
;Usage: getv1,datadir,it,v1,x1_v1,x2_v1,x3_v1,time_v1
;datadir (input): full path of the mfe output data directory
;it (input): time step number
;v1 (return): array for v1
;x1_v1 (return): dimension-1 grid position of the v1 array
;x2_v1 (return): dimension-2 grid position of the v1 array
;x3_v1 (return): dimension-3 grid position of the v1 array
;time_v1 (return): time
;
...
filev1=datadir+'/v1_'+strtrim(string(it,format='(i4.4)'),1)+'.dat'
readdata,filev1,data,time,in,jn,kn
v1=data(is:iep1,js:je,ks:ke)
time_v1=time
