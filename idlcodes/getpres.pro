Pro getpres,datadir,it,pres,x1_pres,x2_pres,x3_pres,time_pres

;Purpose: Read one pressure data array inside domain (no ghost zones) at time step it
;
;Usage: getpres,datadir,it,pres,x1_pres,x2_pres,x3_pres,time_pres
;datadir (input): full path of the mfe output data directory
;it (input): time step number
;pres (return): array for pressure
;x1_pres (return): dimension-1 grid position of the pres array
;x2_pres (return): dimension-2 grid position of the pres array
;x3_pres (return): dimension-3 grid position of the pres array
;time_pres (return): time
;
...
filee=datadir+'/e_'+strtrim(string(it,format='(i4.4)'),1)+'.dat'
readdata,filee,data,time,in,jn,kn
pres=data(is:ie,js:je,ks:ke)*(gamma-1.D0)
time_pres=time
