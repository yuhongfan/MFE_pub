PRO getb2,datadir,it,b2,x1_b2,x2_b2,x3_b2,time_b2

;Purpose: Read one b2 data array inside domain (no ghost zones) at time step it
;
;Usage: getb2,datadir,it,b2,x1_b2,x2_b2,x3_b2,time_b2
;datadir (input): full path of the mfe output data directory
;it (input): time step number
;b2 (return): array for b2
;x1_b2 (return): dimension-1 grid position of the b2 array
;x2_b2 (return): dimension-2 grid position of the b2 array
;x3_b2 (return): dimension-3 grid position of the b2 array
;time_b2 (return): time
;
...
fileb2=datadir+'/b2_'+strtrim(string(it,format='(i4.4)'),1)+'.dat'
readdata,fileb2,data,time,in,jn,kn
b2=data(is:ie,js:jep1,ks:ke)
time_b2=time
